...
export function MonthView({ entries, viewMonth, selectedDay, onMonthChange, onSelectDay }: { entries: Entry[]; viewMonth: Date; selectedDay: string; onMonthChange: (delta: number) => void; onSelectDay: (day: string) => void }) {
  ...
  return <section className="month-section"><div className="section-heading month-heading"><div><p className="eyebrow">the daily view</p><h2>{viewMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h2></div><div className="month-controls"><button onClick={() => onMonthChange(-1)} aria-label="Previous month">‹</button><button onClick={() => onMonthChange(1)} aria-label="Next month">›</button></div></div><div className="month-calendar" aria-label={`${viewMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })} calendar`}><div className="calendar-weekdays">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{calendarCells.map((row, index) => row ? <button key={row.key} type="button" className={`calendar-day ${row.key === selectedDay ? 'selected' : ''} ${row.key === todayKey ? 'today' : ''}`} onClick={() => onSelectDay(row.key)}><span>{row.date.getDate()}</span>{(row.income || row.spent) > 0 && <i />}</button> : <span className="calendar-blank" key={`blank-${index}`} />)}</div></div><div className="month-summary"><span><b>{formatShort(monthIncome)}</b> allocated</span><span><b>{formatShort(monthSpent)}</b> spent</span><span><b>{currentMonthEntries.length}</b> records</span></div><div className="day-list">{visibleDailyRows.map(row => <div key={row.key} className={`day-row ${row.key === todayKey ? 'today' : ''} ${row.key === selectedDay ? 'selected' : ''}`}><span className="day-name">{row.date.toLocaleDateString('en-US', { weekday: 'short' })}</span><span className="day-number">{row.date.getDate()}</span><span className="day-line" /><span className="day-allocation">{row.income ? `+${formatShort(row.income)}` : 'no allocation'}</span><strong className={row.left < 0 ? 'negative' : ''}>{row.income || row.spent ? formatShort(row.left) : '·'}</strong></div>)}</div></section>
}

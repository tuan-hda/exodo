...
export function CategoryPicker({ type, value, onChange, disabled = false }: { type: 'income' | 'expense'; value: Category; onChange: (category: Category) => void; disabled?: boolean }) {
  ...
  return <div className="category-picker"><div className="category-picker-heading"><span>Category</span><span className="category-selected">{categoryIcon(value, 14)} {value}</span></div><div className="category-grid">{options.map(item => <button key={item} disabled={disabled} type="button" className={`category-button ${value === item ? 'selected' : ''}`} aria-label={item} title={item} aria-pressed={value === item} onClick={() => onChange(item)}>{categoryIcon(item, 16)}<span>{item}</span></button>)}</div></div>
}

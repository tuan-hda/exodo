...
export function MobileTabBar({
  ...
  return (
    <nav className="mobile-nav" aria-label="Primary navigation">
      <button className={activeTab === 'today' ? 'active' : ''} type="button" onClick={() => onChange('today')}>
        <House size={20} weight={activeTab === 'today' ? 'fill' : 'regular'} />
        <span>Today</span>
      </button>
      <button className={activeTab === 'month' ? 'active' : ''} type="button" onClick={() => onChange('month')}>
        <CalendarBlank size={20} weight={activeTab === 'month' ? 'fill' : 'regular'} />
        <span>Month</span>
      </button>
      <button className="mobile-nav-record" type="button" onClick={onRecord} aria-label="Add income or expense">

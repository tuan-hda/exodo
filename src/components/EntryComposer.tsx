...
export function EntryComposer({ entry, type, isSaving, dayKey = todayKey, onClose, onSave, onTypeChange }: { entry?: Entry; type: EntryType; isSaving: boolean; dayKey?: string; onClose: () => void; onSave: (entry: Entry) => Promise<boolean>; onTypeChange: (type: EntryType) => void }) {
  ...
  return <Sheet open onOpenChange={open => { if (!open && !isSaving) onClose() }}><SheetContent side="bottom" showCloseButton={false} className={`composer sheet-safe-area ${type}`} aria-busy={isSaving}><SheetTitle className="sr-only">{entry ? `Edit ${type}` : type === 'income' ? 'Income' : 'Expense'}</SheetTitle><div className="sheet-handle" aria-hidden="true" /><div className="composer-heading"><h2 id="composer-title">{entry ? `Edit ${type}` : type === 'income' ? 'Income' : 'Expense'}</h2><div className="composer-actions"><button className="composer-type-indicator" type="button" disabled={isSaving} onClick={toggleType} aria-label={`Switch to ${type === 'income' ? 'expense' : 'income'}`}>{type === 'income' ? <ArrowDown size={14} weight="bold" /> : <ArrowUp size={14} weight="bold" />}{type === 'income' ? 'Income' : 'Expense'}</button><button className="close-composer" type="button" disabled={isSaving} onClick={onClose} aria-label="Close"><X size={19} /></button></div></div><div className="composer-progress" aria-label="Record steps">{stepLabels.map((label, index) => <span className={step === index + 1 ? 'active' : step > index + 1 ? 'complete' : ''} key={label}><i>{step > index + 1 ? <Check size={11} weight="bold" /> : index + 1}</i>{label}</span>)}</div><form onSubmit={submit}>
    {step === 1 && <section className="composer-step" aria-label="Choose category"><CategoryPicker disabled={isSaving} type={type} value={category} onChange={selectedCategory => { setCategory(selectedCategory); setError(''); setStep(2) }} /></section>}
    {step === 2 && <section className="composer-step" aria-label="Enter amount"><label>Amount<input className="amount-input" disabled={isSaving} readOnly={isMobile} autoFocus inputMode="decimal" value={amount} onChange={event => { setError(''); setAmount(formatAmountExpression(event.target.value)) }} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); nextStep() } }} onBlur={() => setAmount(formatAmountExpression(amount))} placeholder="0 or 1200 + 350" /></label><CalculatorKeypad amount={amount} disabled={isSaving} onChange={value => { setAmount(value); setError('') }} onComplete={nextStep} />{error && <p className="form-error" role="alert">{error}</p>}<div className="composer-step-actions"><Button className="step-back" disabled={isSaving} type="button" variant="outline" onClick={previousStep}><ArrowLeft size={17} /> Back</Button></div></section>}
    {step === 3 && <section className="composer-step" aria-label="Review record"><div className="review-card"><div className="review-amount"><span>{type === 'income' ? 'Income' : 'Expense'}</span><strong>{formatMoney(evaluateExpression(amount))}</strong></div><div className="review-row"><span>Category</span><strong>{category}</strong></div></div><label>Name <span className="optional-label">optional</span><input disabled={isSaving} value={title} onChange={event => setTitle(event.target.value)} placeholder={type === 'income' ? 'Salary, bonus...' : 'Coffee, groceries...'} /></label><label>Date and time<input disabled={isSaving} type="datetime-local" value={occurredAt} onChange={event => setOccurredAt(event.target.value)} /></label>{error && <p className="form-error" role="alert">{error}</p>}<div className="composer-step-actions"><Button className="step-back" disabled={isSaving} type="button" variant="outline" onClick={previousStep}><ArrowLeft size={17} /> Back</Button><Button className="submit-record" disabled={isSaving} type="submit" variant="default">{isSaving ? <><CircleNotch className="loading-spinner" size={17} /> Saving…</> : <>{entry ? 'Save changes' : 'Save record'} <Check size={17} weight="bold" /></>}</Button></div></section>}

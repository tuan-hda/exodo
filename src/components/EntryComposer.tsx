...
import { currentTime, evaluateExpression, todayKey } from '../lib/entry-utils'
import type { Entry, EntryType } from '../types/entry'

export function EntryComposer({ entry, type, isSaving, onClose, onSave }: { entry?: Entry; type: EntryType; isSaving: boolean; onClose: () => void; onSave: (entry: Entry) => Promise<boolean> }) {
  ...
  return <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && !isSaving && onClose()}><div className={`composer ${type}`} role="dialog" aria-modal="true" aria-labelledby="composer-title" aria-busy={isSaving}><div className="composer-heading"><div><p className="eyebrow">{entry ? 'edit record' : 'new record'}</p><h2 id="composer-title">{entry ? 'Edit record' : type === 'income' ? 'Add income' : 'Add expense'}</h2></div><div className="composer-actions"><span className="composer-type-indicator">{type === 'income' ? <ArrowDown size={14} weight="bold" /> : <ArrowUp size={14} weight="bold" />}{type === 'income' ? 'Income' : 'Expense'}</span><button className="close-composer" disabled={isSaving} onClick={onClose} aria-label="Close"><X size={19} /></button></div></div><form onSubmit={submit}><label>Amount<input className="amount-input" disabled={isSaving} readOnly={isMobile} autoFocus inputMode="decimal" value={amount} onChange={event => setAmount(event.target.value)} placeholder="0 or 1200 + 350" /></label><CalculatorKeypad amount={amount} disabled={isSaving} onChange={value => { setAmount(value); setError('') }} /><label>Name<input disabled={isSaving} value={title} onChange={event => setTitle(event.target.value)} placeholder={type === 'income' ? 'Salary, freelance...' : 'Coffee, groceries...'} /></label><CategoryPicker disabled={isSaving} type={type} value={category} onChange={setCategory} /><label>Date and time<input disabled={isSaving} type="datetime-local" value={occurredAt} onChange={event => setOccurredAt(event.target.value)} /></label>{type === 'income' && <p className="allocation-note">Income dated on or before the 15th is divided from that date through month end. Later income starts next month.</p>}{error && <p className="form-error">{error}</p>}<button className="submit-record" disabled={isSaving} type="submit">{isSaving ? <><CircleNotch className="loading-spinner" size={15} /> Saving…</> : <>{entry ? 'Save changes' : 'Save record'} <ArrowUp size={16} /></>}</button></form></div></div>
}

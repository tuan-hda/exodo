...
export function ActivityList({ entries, todayKey, deletingEntryId, onEdit, onRemove }: { entries: Entry[]; todayKey: string; deletingEntryId: string | null; onEdit: (entry: Entry) => void; onRemove: (id: string) => void }) {
  ...
  const groupedEntries = filteredEntries.reduce<ActivityMonth[]>((groups, entry) => {
    ...
    if (group) {
      ...
      groups.push({ key, label: fromKey(`${key}-01`).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }), entries: [entry], income: entry.type === 'income' ? entry.amount : 0, expense: entry.type === 'expense' ? entry.amount : 0 })
    }
    return groups
  }, [])
  ...
  return <section className="activity-section"><div className="section-heading activity-heading"><div><p className="eyebrow">recent activity</p><h2>What moved.</h2></div><ClockCounterClockwise size={22} /></div>{entries.length > 0 && <div className="activity-tools"><label className="activity-search"><MagnifyingGlass size={16} /><span className="sr-only">Search activity</span><input value={query} onChange={event => updateQuery(event.target.value)} placeholder="Search activity" /></label><span className="activity-count">{filteredEntries.length} {filteredEntries.length === 1 ? 'record' : 'records'}</span></div>}{monthKeys.length > 0 && <nav className="activity-date-nav" aria-label="Activity months"><button className="activity-date-option" disabled={!previousMonth} type="button" onClick={() => previousMonth && setSelectedMonth(previousMonth)} aria-label={previousMonth ? `Previous month, ${monthChip(previousMonth)}` : 'No previous month'}><CaretLeft size={17} /><span>{previousMonth ? monthChip(previousMonth) : '—'}</span></button><div className="activity-date-current" aria-current="date"><span>Viewing</span><strong>{activeMonth ? monthChip(activeMonth) : '—'}</strong></div><button className="activity-date-option next" disabled={!nextMonth} type="button" onClick={() => nextMonth && setSelectedMonth(nextMonth)} aria-label={nextMonth ? `Next month, ${monthChip(nextMonth)}` : 'No next month'}><span>{nextMonth ? monthChip(nextMonth) : '—'}</span><CaretRight size={17} /></button></nav>}<div className="activity-list">{activeGroup ? <section className="activity-day"><h3>{activeGroup.label}</h3><div className="activity-month-summary" aria-label={`${activeGroup.label} totals`}><span><b>Income</b><strong className="income">+{formatShort(activeGroup.income)}</strong></span><span><b>Expense</b><strong className="expense">-{formatShort(activeGroup.expense)}</strong></span><span><b>Leftover</b><strong className={`leftover ${activeGroup.income - activeGroup.expense < 0 ? 'negative' : ''}`}>{formatShort(activeGroup.income - activeGroup.expense)}</strong></span></div>{groupByDate(activeGroup.entries).map(day => <section className="activity-date-group" key={day.key}><h4>{day.label}</h4>{day.entries.map(entry => <div className="activity-row" key={entry.id} onClick={() => onEdit(entry)} role="button" tabIndex={0} onKeyDown={event => event.key === 'Enter' && onEdit(entry)}><span className={`activity-icon ${categoryClass(entry.category ?? 'Other')}`}>{categoryIcon(entry.category ?? 'Other', 16)}</span><span className="activity-name"><strong>{entry.title || entry.category || (entry.type === 'income' ? 'Income' : 'Expense')}</strong><small>{entry.category ?? 'Other'} · {entry.occurredAt.slice(11, 16)}</small></span><b className={entry.type === 'income' ? 'income-text' : ''}>{entry.type === 'income' ? '+' : '-'}{formatShort(entry.amount)}</b><button className="remove-entry" disabled={deletingEntryId === entry.id} onClick={event => { event.stopPropagation(); onRemove(entry.id) }} aria-label={`Remove ${entry.title || entry.category || entry.type}`}>{deletingEntryId === entry.id ? <CircleNotch className="loading-spinner" size={15} /> : <X size={15} />}</button></div>)}</section>)}<p className="activity-end">End of transactions</p></section> : <div className="empty-activity"><CalendarDots size={22} /><p>{entries.length ? 'No matching records found.' : 'Your first record will show up here.'}</p></div>}</div></section>
}

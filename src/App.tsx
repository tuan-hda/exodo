...
function App() {
  ...
  return <div className="app-shell"><main id="top" className="page">
    ...
    {activeTab === 'today' && <><section className="intro"><div><p className="eyebrow">{currentDay.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p><h1>Spend what today<br /><em>makes possible.</em></h1></div><p className="intro-copy">Income becomes a daily allowance. Each expense makes the rest of today visible.</p></section><SummaryPanels entries={entries} accumulation={accumulation} dayKey={currentDayKey} /><div className="home-activity"><ActivityList entries={entries} deletingEntryId={deletingEntryId} onEdit={entry => openComposer(entry.type, entry)} onRemove={removeEntry} /></div></>}

...
function App() {
  ...
  return <div className="app-shell"><header className="topbar"><a className="wordmark" href="#top" aria-label="Exodo home"><span className="wordmark-mark">e</span><span>exodo</span></a><p className="topbar-note">personal money, in motion</p><div className="topbar-actions"><AuthAccount /><button className="topbar-add" onClick={() => openComposer('expense')}><Plus size={16} weight="bold" /> Record</button></div></header><main id="top" className="page">
    ...
    <SummaryPanels entries={entries} accumulation={accumulation} />
    ...
    <MonthView entries={entries} viewMonth={viewMonth} selectedDay={selectedDay} onMonthChange={moveMonth} onSelectDay={setSelectedDay} />
    <ActivityList entries={entries} deletingEntryId={deletingEntryId} onEdit={entry => openComposer(entry.type, entry)} onRemove={removeEntry} />
  </main><footer className="footer"><span>exodo / έξοδο</span><span>money is a daily practice</span></footer>{composerOpen && <EntryComposer key={editingEntry?.id ?? composerType} entry={editingEntry} type={composerType} isSaving={isSaving} onClose={() => { setComposerOpen(false); setEditingEntry(undefined) }} onSave={handleSave} />}</div>
}

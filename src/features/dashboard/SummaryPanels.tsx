    ...
    <section className="summary-grid" aria-label="Money summary">
      <section className={`today-panel ${availableToday < 0 ? 'is-over' : ''}`} aria-label="Today's available amount">
        <div className="today-copy">
          <span className="today-label">available today</span>
        ...
        <div className="today-mark">
          <span>{availableToday < 0 ? 'over pace' : 'on pace'}</span>
      ...
      <section className="accumulation-panel" aria-label="All-time accumulation">
        <div className="accumulation-copy">
          <span className="today-label">accumulated</span>
          <strong>{accumulation === null ? '—' : formatMoney(accumulation)}</strong>
          <span className="today-detail">all income minus all expenses</span>
        </div>
